...
type UserStore interface {
	...
	GetUserToken(scopre, tokePlainText string) (*User, error)
}
...
func (s *PostgresUserStore) GetUserToken(scope, tokenPlainText string) (*User, error) {
	...
	err := s.db.QueryRow(query, scope, tokenHash[:], time.Now()).
		Scan(&user.ID, &user.UserName, &user.Email, &user.PasswordHash.hash, &user.Bio, &user.CreatedAt, &user.UpdatedAt)
